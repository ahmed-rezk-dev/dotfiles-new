...
module.getHighlightWindowColor = function()
    ...
    local highlightBorderColor

    if config.window.highlightBorderColor == nil then
        highlightBorderColor = blueColor
    else
        highlightBorderColor = config.window.highlightBorderColor
    end
    return cache.osxApperance == "graphite" and grayColor or highlightBorderColor
end
...
module.drawBorder = function()
    ...
    local borderWidth = 8
    local distance = 6
    local roundRadius = 28

    local isFullScreen = focusedWindow:isFullScreen()
    local frame = focusedWindow:frame()

    if not cache.borderCanvas then
        cache.borderCanvas = hs.canvas.new({ x = 0, y = 0, w = 0, h = 0 }):level(hs.canvas.windowLevels.overlay):behavior({ hs.canvas.windowBehaviors.transient, hs.canvas.windowBehaviors.moveToActiveSpace }):alpha(alpha)
    end
